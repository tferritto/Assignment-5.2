...
void CopySingletonsGatherDups(Node*& head1, Node*& head2)
{
	...
	Node* currentHead = head1;
	Node* currentNode = head1->link;
	Node* prevNode = head1;
	...
	while(currentHead != 0){
		...
		while(currentNode != 0){
			if( currentNode->data == currentHead->data){
				isUnique = false;
				...
				if ( destList==0 )
					destList = destTail = currentNode;
				else{
					destTail->link = currentNode;
					destTail = currentNode;
				}
		...
		if ( isUnique ){   //copy to the destination list
		...
		if ( currentHead->link == 0 ){
            
            head2 = destList; // head2 points to destList
			return;
        }
        
		currentHead = currentHead->link;
		prevNode = currentHead;
		currentNode = currentHead->link;
	}
}
